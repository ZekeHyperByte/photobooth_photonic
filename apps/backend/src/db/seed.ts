...
async function seed() {
  try {
    ...
    const samplePhotoPath = path.join(process.cwd(), 'data', 'sample-photo.jpg');
    ...
    const filterValues = await Promise.all(
      Object.entries(FILTER_PRESETS).map(async ([key, preset], index) => {
        ...
        if (fs.existsSync(samplePhotoPath)) {
          thumbnailPath = path.join(thumbnailsDir, `filter-thumb-${filterId}.jpg`);
          try {
            await imageProcessor.generateFilterPreview(preset.config, thumbnailPath);
            logger.info(`Generated thumbnail for filter: ${preset.name}`);
          } catch (error) {
            logger.warn(`Failed to generate thumbnail for filter ${preset.name}`, { error });
            thumbnailPath = null;
          }
        }
    ...
    // Ensure directories exist (thumbnailsDir already created in filter seeding)
    ...
    const defaultTemplates = [
      {
        id: 'main-template',
        name: 'Main Template',
        description: 'Newspaper layout: 1 large photo on top, 2 smaller photos on bottom',
    ...
    for (const template of defaultTemplates) {
      ...
      if (fs.existsSync(samplePhotoPath)) {
        ...
        try {
          ...
          logger.warn(`Failed to generate preview for ${template.name}, skipping...`);
          previewPath = null;
        }
      } else {

...
export const sessionsRelations = relations(sessions, ({ one, many }) => ({
  ...
  boothCode: one(boothCodes, {
    fields: [sessions.boothCodeId],
    references: [boothCodes.id],
  }),
  photos: many(photos),
...
// Booth codes relations
export const boothCodesRelations = relations(boothCodes, ({ many }) => ({
  sessions: many(sessions),
}));
...
// Print queue relations
export const printQueueRelations = relations(printQueue, ({ one }) => ({
  photo: one(photos, {
    fields: [printQueue.photoId],
    references: [photos.id],
  }),
  session: one(sessions, {
    fields: [printQueue.sessionId],
    references: [sessions.id],
  }),
}));

// WhatsApp deliveries relations
export const whatsappDeliveriesRelations = relations(
  whatsappDeliveries,
  ({ one }) => ({
    photo: one(photos, {
      fields: [whatsappDeliveries.photoId],
      references: [photos.id],
    }),
    session: one(sessions, {
      fields: [whatsappDeliveries.sessionId],
      references: [sessions.id],
    }),
  }),
);

// Audit logs relations (no relations needed)
